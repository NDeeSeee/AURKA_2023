...
# Functions --------------------------------------------------------------------
parse_cbioportal_data <- function(sample_data, patient_data, alterations_data, rna_seq_data, custom_columns) {
  
  custom_columns <- custom_columns
  
  sample_data <- fread(sample_data,
                       skip = 4, na.strings = c("", "[Not Available]")) %>% 
    as_tibble()
  ...
  alterations_data <- fread(alterations_data, na.strings = c("", "[Not Available]")) %>% 
    as_tibble() %>% 
    mutate(sample_id = `Sample ID`, study_id = `Study ID`) %>% 
    select(sample_id, study_id, TP53, EGFR, KRAS, AURKA) %>% 
    mutate(across(.cols = matches("TP53|EGFR|KRAS|AURKA"), .fns = function(x) ifelse(x == "no alteration", "WT", "ALT")))
  ...
  clinical_data <- clinical_data %>% 
    ...
    mutate(across(.cols = matches("exp|tmb|age|months|smoking_pack_years"), .fns = function(x) as.numeric(x)),
           across(.cols = matches("sex"), .fns = function(x) tolower(x)),
           across(.cols = matches("smoking_history"), .fns = function(x) as.character(x)))
  
  return(clinical_data)
}

custom_columns <-  c("PATIENT_ID" = "patient_id", 
                     "SAMPLE_ID" = "sample_id",
...
OncoSG_data <- parse_cbioportal_data("OncoSG lung adenocarcinoma data/data_clinical_sample.txt",
...
CPTAC_data <- parse_cbioportal_data("CPTAC lung adenocarcinoma data/data_clinical_sample.txt",
...
TCGA_data_extended <- parse_cbioportal_data("TCGA pancancer lung adenocarcinoma data/data_bcr_clinical_data_sample.txt",
...
TCGA_data <- parse_cbioportal_data("TCGA pancancer lung adenocarcinoma data/data_clinical_sample.txt",
                                   "TCGA pancancer lung adenocarcinoma data/data_clinical_patient.txt",
                                   "TCGA pancancer lung adenocarcinoma data/alterations_across_samples.tsv",
                                   "TCGA pancancer lung adenocarcinoma data/mRNA expression z-scores relative to all samples (log RNA Seq V2 RSEM).txt",
                                   custom_columns) %>% 
  left_join(y = select(TCGA_data_extended, sample_id, smoking_history, smoking_pack_years))


merged_data <- bind_rows(OncoSG_data, CPTAC_data) %>% 
  bind_rows(TCGA_data) %>% 
  mutate(stg = str_remove(stg, "STAGE "),
         stg = str_remove(stg, "[A|B]"),
         stg = case_when(stg == "I" ~ "1",
                         stg == "II" ~ "2",
                         stg == "III" ~ "3",
                         stg == "IV" ~ "4",
                         str_detect(stg, "1") ~ "1",
...
write.csv(merged_data, "Merged annotated data AURKA, KRAS, TP53, EGFR.csv", row.names = F)

# Analysis from 04.05.2023 -----------------------------------------------------
sample_data <- fread("TCGA pancancer lung adenocarcinoma data/data_clinical_sample.txt",
                       skip = 4, na.strings = "") %>% 
  as_tibble() %>% 
  rename(sample_id = SAMPLE_ID, patient_id = PATIENT_ID, tmb = TMB_NONSYNONYMOUS) %>% 
  select(sample_id, patient_id, tmb)

clinical_data <- fread("TCGA pancancer lung adenocarcinoma data/data_clinical_patient.txt",
                       skip = 4, na.strings = "") %>% 
  as_tibble() %>% 
  rename(patient_id = PATIENT_ID, stage = AJCC_PATHOLOGIC_TUMOR_STAGE,
         age = AGE, sex = SEX, race = RACE, rad_therapy = RADIATION_THERAPY,
...
protein_exp_data <- fread("TCGA pancancer lung adenocarcinoma data/Protein expression z-scores (RPPA).txt") %>% 
  as_tibble() %>% 
  filter(!if_all(
    .cols = c("TP53", "EGFR", "KRAS", "AURKA"),
    .fns = function(x)
...
alterations_data <- fread("TCGA pancancer lung adenocarcinoma data/alterations_across_samples.tsv") %>% 
  as_tibble() %>% 
  mutate(sample_id = `Sample ID`, study_id = `Study ID`) %>% 
  select(sample_id, study_id, TP53, EGFR, KRAS, AURKA) %>% 
  mutate(across(.cols = matches("TP53|EGFR|KRAS|AURKA"), .fns = function(x) ifelse(x == "no alteration", "WT", "ALT")))
...
annotated_data %>% 
  select(contains("rna")) %>%
    pivot_longer(cols = everything(), names_to = "hugo_symbol", values_to = "mrna_exp_zscore") %>% 
    mutate(hugo_symbol = str_remove_all(hugo_symbol, "_rna_exp"),
...
annotated_data %>% 
  ...
  mutate(EGFR_only = ifelse(EGFR == "ALT" & KRAS == "WT", "ALT", "WT"),
         KRAS_only = ifelse(KRAS == "ALT" & EGFR == "WT", "ALT", "WT"),
         across(!AURKA_rna_exp, .fns = function(x) as.factor(x))) %>% 
  pivot_longer(cols = contains("only"), names_to = "subgroup", values_to = "alteration") %>% 
  filter(!alteration == "WT") %>% 
  ggplot(aes(y = AURKA_rna_exp, fill = subgroup, x = subgroup, col = TP53)) +
